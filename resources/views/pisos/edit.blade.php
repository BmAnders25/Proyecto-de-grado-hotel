...
@section('content_header')
    <h1 class="m-0 text-dark">Editar Piso</h1>
@stop

@section('content')
    <x-adminlte-card>
        <form method="POST" action="{{ route('pisos.update', $piso->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <x-adminlte-input name="nombre" label="Nombre" placeholder="Nombre del Gasto"
                    fgroup-class="col-md-6" :value="$piso->nombre" />
            </div>
            <div class="row">
                <x-adminlte-select name="estado" label="Estado del Gasto" fgroup-class="col-md-2">
                    ...
                    <option value="Activo" {{ $piso->estado === 'Activo' ? 'selected' : '' }}>Activo</option>
                    <option value="Inactivo" {{ $piso->estado === 'Inactivo' ? 'selected' : '' }}>Inactivo</option>
                </x-adminlte-select>
            </div>
